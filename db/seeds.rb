...
5.times do |i|
  id = i + 1
  pb = Publisher.create(name: "#{Faker::Book.unique.publisher}")
  g = Genre.create(name: "#{Faker::Book.unique.genre}")
  pl = Platform.create(name: "#{Faker::Ancient.unique.primordial}")
  gm = Game.create(title: "#{Faker::App.unique.name}", description: "#{Faker::StarWars.wookie_sentence}", publisher: pb, genre: g)
  pg = PlatformGame.create(game: gm, platform: pl)
